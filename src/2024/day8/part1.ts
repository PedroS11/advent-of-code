...
const printAntiNodes = (antinodes: Set<string>, maze: string[][]) => {
  const solvedMaze = JSON.parse(JSON.stringify(maze));
    ...
    locations.forEach((location, index) => {
      ...
      locations.forEach(newLocation => {
        if (index !== startingIndex) {
          ...
          if (basicMaze[sY - deltaY]?.[sX - deltaX]) {
            antinodes.add(`${sX - deltaX},${sY - deltaY}`);
          }
